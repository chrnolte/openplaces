    ...
    df = pd.DataFrame(
        {
            'postal_code': ['01862'],
            'city': [pd.NA],
            'address': ['1 Sample Ave, MA 01862'],
            'address_number': ['1'],
            'address_street': ['SAMPLE AVE'],
            'address_unit': [pd.NA],
        }
    )
    state = impute_postal_city(make_state(df))
...
def test_impute_postal_city_does_not_override_existing_city():
    df = pd.DataFrame(
        {
            'postal_code': ['01862'],
            'city': ['Billerica'],
            'address': ['1 Sample Ave, Billerica, MA 01862'],
            'address_number': ['1'],
            'address_street': ['SAMPLE AVE'],
            'address_unit': [pd.NA],
        }
    ...
    assert res.loc[0, 'address'] == '1 Sample Ave, Billerica, MA 01862'
...
def test_impute_postal_city_backfill_needs_no_address_number_or_unit_columns():
    ...
    assert res.loc[0, 'city'] == 'North Billerica'
    assert res.loc[0, 'address'] == 'Sample Ave, North Billerica, MA 01862'
