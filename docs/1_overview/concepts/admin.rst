...
The default administrative unit hierarchy and geometries are built entirely from permissively licensed, public open-data sources (such as Eurostat's GISCO, U.S. Census Bureau, UK ONS, and Colombia DANE) loaded via the spine generation recipes (:gh-file:`src/openplaces/recipes`). This open-data pipeline ensures that the entire database is fully compatible with the package's Apache-2.0 license, avoiding the commercial use restrictions associated with datasets like GADM.

Administrative identifiers are generated by the ``openplaces`` candidate generation and weighted assignment optimization pipeline. They anchor on standard ISO 3166-2 codes at the country and state levels, and are derived algorithmically from unit names to optimize readability and uniqueness.
...
.. admonition:: Disclaimer
   ...
   The default spine is compiled from official and public administrative registries and is used for convenience, not as a political stance.
